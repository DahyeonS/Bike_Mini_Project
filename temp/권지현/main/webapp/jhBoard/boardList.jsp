...
function getCount () {
	...
	$.ajax({	        
		type: 'post',
        url: '../jhBoard/selectCount.json',
        data: params,
        dataType: 'json',
        success: function(data) {
        	str = '<h8 class="head" align ="center"> &nbsp; &nbsp; 총 게시물 수 : ' + data['totalCount'] + '</h8>'
            $('#totalCount').html(str);
        },
...
function searchList(pageNum) {	
	if(pageNum == null) {
   	...
   	$.ajax({
       type: 'post',
       url: '../jhBoard/search.json',
       data: params,
       dataType: 'json',
       success: function(data) {
       	str ='';
       	for(item of data) {
       		str += '<tr class="table-danger" align="center"><td width ="10%" class="table-danger">' + item['num'] + '</td><td width ="10%">' + item['postdate'] + '</td><td width ="10%">' + item['nickname'] + '</td><td width ="*">' + '<a href="../jhBoard/view.do?num=' + item.num +'">' + item['title'] + '</a>' + '</td><td width ="10%">' + item['visitCount'] +'</td></tr>';      	
       	}
       	$('#boardList').html(str);
       	getCount();
       },
...
<tr align="center">
	...
	<tr class="table-info">
		<td align ="center" class="table-info">
			<select id="searchField" class="table-info">
				<option value ="title">제목</option>
				<option value ="category">닉네임</option>
			</select>
			<input type ="text" id="searchWord" class="table-info">
