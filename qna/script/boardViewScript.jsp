    ...
    $.ajax({
        ...
        success: function(data) {
        	if (data[0]['num'] !== 0) {
	        	let string = '';
	        		for (item of data) {
		        		string += '<br><br><br><h2 style="font-weight: bold;">' + item['title'] + '</h2><h4 id="anickname">작성자 ' + item['nickname'] + '</h4><h4 id="apostdate">작성일자 ' + item['postdate'] + '</h4>';
		        		if (item['updateDate'] !== "0") string += '<h4 id="aupdatedate">수정일자 ' + item['updateDate'] + '</h4>';
		        		string += '<hr><h3 id="acontext">' + item['context'] + '</h3>';
